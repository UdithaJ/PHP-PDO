...
function loadPos($conn,$profile_id){
    $stmt = $conn->prepare("SELECT * FROM Position WHERE profile_id = $profile_id  ORDER BY `rank` ");
...
function insertEdu($conn,$profile_id){
  ...
  for($i=1; $i<=9; $i++) {
    ...
    $stmt = $conn->prepare('INSERT INTO Education(profile_id,`rank`, year, institution_id) VALUES ( :pid, :rank, :year, :iid)');
    $stmt->execute(array(
      ':pid' => $profile_id,
      ':rank' => $rank,
      ':year' => $year,
